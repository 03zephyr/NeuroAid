...
st.markdown("### What is NeuroAid?")
st.markdown("""
**NeuroAid** is an AI-powered application designed to assist patients and clinicians in assessing the risk of Alzheimer's disease. It provides both quick, accessible screening using minimal, easy-to-obtain features and advanced diagnostic support using MRI scans. NeuroAid aims to make early detection, risk explanation, and personalized advice available to everyone.
""")

# --- New Section: What Does NeuroAid Use? ---
st.markdown("### What does NeuroAid use?")
st.markdown("""
- **Minimal Clinical Features:**  
  NeuroAid can predict Alzheimer's risk using a small set of straightforward clinical and lifestyle features, making it accessible and non-invasive.

- **MRI-Based AI Diagnosis:**  
  For more detailed assessment, users can upload MRI scans. The app uses deep learning to analyze the scans and provide a diagnosis.

- **Explainable AI:**  
    - **SHAP (SHapley Additive exPlanations):** Explains how each feature influenced the risk prediction, making the results transparent and understandable.
    - **Grad-CAM (Gradient-weighted Class Activation Mapping):** Highlights regions of MRI scans that most influenced the model's decision, providing visual explanations for clinicians and patients.

- **LLM Integration:**  
  After a prediction, a large language model (LLM) explains the results in plain language and offers personalized lifestyle advice to help manage or reduce risk.
""")

st.divider()


st.markdown("### Input Features Information")
st.text("")
